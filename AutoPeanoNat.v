...
Lemma pred_succ (n : nat) : Init.Nat.pred (S n) = n.
Proof.
scope_nat.
Abort.

Lemma pred_0 : Init.Nat.pred 0 = 0.
Proof.
snipe.
Qed.
...
Lemma sub_succ_r n m : n - (S m) = Init.Nat.pred (n - m).
Proof.
revert m. scope. eliminate_dependent_pattern_matching H4. induction n; destruct m; snipe sub_0_r.
Qed. (* TODO : quand une des branches du match est aussi un pattern matching, il faut recommencer l'opération *)
...
Proof.
assert (succ_r : forall x y, x+S y = S(x+y)) by (induction x ; snipe).
assert (comm : forall x y, x+y = y+x).
{ induction x; destruct y. scope. scope. verit. scope. verit.

 simpl; auto. intros; rewrite succ_r; now f_equal. }
now rewrite comm.
Qed.
