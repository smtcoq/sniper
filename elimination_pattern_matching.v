...
MetaCoq Quote Recursively Definition nat_reif := list.
Print mutual_inductive_body.
...
match l with 
| nil => nil
| ((_, Ty), _):: l' => (tApp (subst10 T Ty) args)  :: (subst_type_constructor_list l' p)
end.


Definition list_types_of_each_constructor t :=
let v := (type_no_app t.2) in
let x := get_decl_inductive v.1 t.1 in match x with
| Some y => match y with 
          | nil => nil
          | cons y' l => let z := y'.(ind_ctors) in let u := 
...
Compute list_types_of_each_constructor list_reif.
...
| Some J => let ind := J.1.1 in let inst := J.1.2 in let args := J.2 in let fix aux n' ind' inst' args :=
match n' with 
| 0 => []
...
pose l_ctors as l_ctor ;
pose l_ty_ctors as l_ty ;
