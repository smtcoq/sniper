...
Definition mk_mind_entry_compdec 
  (mind : mutual_inductive_body) 
  (trm : term) 
  (idt : string) 
  (lpars : list term) : mutual_inductive_entry*(list (term*(list term))) := 
...
Definition gen_compdec (p : term*(list term)) :=
  match p with
    | (t, x::xs) => 
              if contains_trel (x::xs) then 
                let n := Datatypes.length (x::xs) in
                let l := (get_list_of_rel n) in 
                (gen_compdec_statement_aux2 (tApp CompDec_reif [tApp t l]) l (S (S n)), n)
...
Definition add_compdec_inductive_and_pose_compdecs_lemmas 
(t : term)  := 
  match t with
   | tInd ind0 _ =>
     decl <- tmQuoteInductive (inductive_mind ind0) ;; 
