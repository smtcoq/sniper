...
Tactic Notation "snipe" constr(h) :=
  let tac := ltac2:(h |- Control.enter (fun () =>
    intros;
    let l := global_of_ltac1_constr h in
    let l' := List.map (fun t => (None, t)) l in
    let _ := pose_hyps l' [] in scope ();
    ltac1:(verit_nocompdecs)
    ))
  in
  tac h.
...
Tactic Notation "snipe_no_check" constr(h) :=
  let tac := ltac2:(h |- Control.enter (fun () =>
    ...
    let l' := List.map (fun t => (None, t)) l in
    let _ := pose_hyps l' [] in scope ();
    ltac1:(verit_no_check_nocompdecs)
    )) in
  tac h.
...
Tactic Notation "snipe_timeout" constr(h) int_or_var(timeout) :=
  let tac := ltac2:(h t |- Control.enter (fun () =>
    intros;
    let l := global_of_ltac1_constr h in
    let l' := List.map (fun t => (None, t)) l in
    let _ := pose_hyps l' [] in scope ();
    ltac1:(t' |- verit_nocompdecs_timeout t') t
    ))
  in
  tac h timeout.
...
Tactic Notation "snipe_no_check_timeout" constr(h) int_or_var(timeout) :=
  let tac := ltac2:(h t |- Control.enter (fun () =>
    ...
    let l' := List.map (fun t => (None, t)) l in
    let _ := pose_hyps l' [] in scope ();
    ltac1:(t' |- verit_no_check_nocompdecs_timeout t') t
    ))
  in
  tac h timeout.
