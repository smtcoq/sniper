...
Lemma dummy_length : forall A (l : list A), Datatypes.length l = match l with | nil => 0 | cons x xs => S (Datatypes.length xs) end.
intros. destruct l; simpl;  reflexivity. Qed.

Lemma test_length: (False -> forall A (l : list A), Datatypes.length l = match l with | nil => 0 | cons x xs => S (Datatypes.length xs) end) 
/\ True.
